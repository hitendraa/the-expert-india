"use client";

import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";
import { Badge } from "@/components/ui/badge";
import { User, FileText, Award, Copy } from "lucide-react";
import { TRADEMARK_RENEWAL_DOCUMENTS } from "@/lib/services-constants";
import "@/app/animations.css";

const iconMap = {
  User,
  FileText,
  Award,
  Copy
};

const TrademarkRenewalDocuments = () => {
  return (
    <section className="py-8 md:py-12 lg:py-16 bg-gray-50">
      <div className="container mx-auto px-4">
        {/* Section Header */}
        <div className="text-center mb-6 md:mb-8 lg:mb-12">
          <Badge className="bg-brand-primary/10 text-brand-primary hover:bg-brand-primary/20 mb-4">
            <FileText className="h-4 w-4 mr-2" />
            Documents Required
          </Badge>
          <h2 className="text-xl md:text-2xl lg:text-3xl font-bold text-gray-900 mb-3">
            Documents Required for Trademark Renewal
          </h2>
        </div>

        {/* Documents Grid */}
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {TRADEMARK_RENEWAL_DOCUMENTS.map((doc, index) => {
            const IconComponent = iconMap[doc.icon as keyof typeof iconMap];
            
            return (
              <Card key={index} className="animated-hover shadow-md border-0 h-full">
                <CardHeader className="pb-4">
                  <div className="bg-brand-primary/10 p-3 rounded-full w-fit mx-auto mb-3">
                    <IconComponent className="h-6 w-6 text-brand-primary" />
                  </div>
                  <CardTitle className="text-center">
                    <div className="text-lg font-bold text-brand-primary mb-1">
                      {doc.title}
                    </div>
                    <div className="text-base font-semibold text-gray-900">
                      {doc.subtitle}
                    </div>
                  </CardTitle>
                </CardHeader>
                <CardContent className="text-center">
                  <p className="text-gray-600 text-sm leading-relaxed">
                    {doc.description}
                  </p>
                </CardContent>
              </Card>
            );
          })}
        </div>
      </div>
    </section>
  );
};

export default TrademarkRenewalDocuments;
